...
// Address test
func TestTransactionsTokenTransferAddressEndpoint(t *testing.T) {
	assert := assert.New(t)
	...
	// Test number
	resp, err = http.Get(transactionsServiceURL + transactionsServiceRestPrefx + "/transactions/token-transfers/address/" + transactionFromAddress)
	assert.Equal(nil, err)
	assert.Equal(200, resp.StatusCode)
