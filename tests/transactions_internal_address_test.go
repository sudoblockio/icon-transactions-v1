...
// Address test
func TestTransactionsEndpointInternalAddress(t *testing.T) {
	assert := assert.New(t)
	...
	resp, err := http.Get(transactionsServiceURL + transactionsServiceRestPrefx + "/transactions?type=internal&limit=1")
	...
	// Test number
	resp, err = http.Get(transactionsServiceURL + transactionsServiceRestPrefx + "/transactions/internal/address/" + transactionFromAddress)
	assert.Equal(nil, err)
	assert.Equal(200, resp.StatusCode)
