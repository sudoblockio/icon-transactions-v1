...
func convertBsonMToTransaction(m bson.M) models.Transaction {
  ...
  return models.Transaction {
    ...
    StepLimit: uint64(m["steplimit"].(int64)),
    ...
    Fee: uint64(m["fee"].(int64)),
    Signature: m["signature"].(string),
    DataType: m["datatype"].(string),
    Data: data,
    ReceiptCumulativeStepUsed: uint64(m["receiptcumulativestepused"].(int64)),
    ReceiptStepUsed: uint64(m["receiptstepused"].(int64)),
    ReceiptStepPrice: uint64(m["receiptstepprice"].(int64)),
